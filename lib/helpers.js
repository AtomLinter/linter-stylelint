...
function requireCached(request) {
  ...
  moduleCache.set(modulePath, moduleInstance);
  return moduleInstance;
}

export function getStylelintPath(filePath) {
  try {
    return resolve('stylelint', { basedir: path.dirname(filePath) });
  } catch (e) {
...
export function getStylelintInstance(filePath) {
  const stylelintPath = getStylelintPath(filePath);
  ...
  return requireCached(stylelintPath);
}
